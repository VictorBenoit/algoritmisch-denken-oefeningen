...
function baz(x, y) {
    let a = x ** 2;
    let b = y ** 2;
    return (a + b) ** 0.5; // ** 0.5 is equivalent met vierkantswortel
}
...
function cake3(eggs, flour, butter, sugar) {
    let maxEggs = Math.floor(eggs / 4);
    let maxFlour = Math.floor(flour / 250);
    let maxSugar = Math.floor(sugar / 250);
    let maxButter = Math.floor(butter / 250);
    return Math.min(maxEggs, maxFlour, maxButter, maxSugar);
}
